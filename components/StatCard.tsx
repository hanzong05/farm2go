...
interface StatCardProps {
  ...
  subtitle?: string;
  growth?: number;
  variant?: 'default' | 'bordered' | 'horizontal';
}
...
export default function StatCard({
  ...
  if (variant === 'horizontal') {
    return (
      <View style={styles.horizontalStatCard}>
        <Text style={[styles.horizontalStatValue, { color }]}>{value}</Text>
        <Text style={styles.horizontalStatTitle}>{title}</Text>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  // Horizontal variant styles (like your first image)
  horizontalStatCard: {
    flex: 1,
    alignItems: 'center',
    paddingVertical: 16,
